...
//CRC-8 多项式x8+x2+x+1 查表算法
static const u8 crc_array[256] = {
...
u8 calc_crc8(u8 *pbuf, u8 len)
{
	u8 crc8 = 0,i;
	
	for (i=0; i<len; i++) {	
		crc8 = crc_array[crc8^pbuf[i]]; //查表得到CRC码
	}
	return crc8;
}
...
u8 CRC8_Sum(u8 *data, u8 num )
{
   int buf = 0; 
    ...
    for (int i = 0; i < num; i++)
    {
        buf = (buf + data[i]) % 0xFFFF;
    }
    u8 ret = ( u8)(buf & 0xff);//只要最后一个字节
    return ret;
}
